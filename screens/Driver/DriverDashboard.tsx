...
export default function DriverDashboard() {
  ...
  const fetchAllParcels = useCallback(async () => {
    ...
    try {
      ...
      const url = `https://tanmia-group.com:86/courierApi/parcels/DriverParcels/${intSenderEntityCode}`;
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `https://tanmia-group.com:86/courierApi/driverparcels/DashboardData/${userId}`
      );
      if (response.data) {
